...
def main():
    ...
    for i in range(num):
        ...
        # ========== 优化1：规范路径拼接（避免四重反斜杠、兼容所有系统） ==========
        img_name = f"{i}-{mtime}.jpg"  # 格式化文件名，更简洁
        img_path = os.path.join(save_root, img_name)  # 自动拼接路径，无冗余符号
        img.save(img_path)  # 保存图片到本地
